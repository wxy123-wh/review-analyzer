...
public class ActionRepository {
    ...
    public ActionResponse create(ActionCreateRequest request) {
        ...
        payload.put(
                ...
                issueCluster == null
                        ? null
                        : resolveDefaultLaunchedAt(
                                productId,
                                issueCluster.uxSecondaryLabel(),
                                issueCluster.aspect()
    ...
    private Optional<IssueClusterRef> findIssueClusterRef(long productId, String issueId) {
        ...
        List<IssueClusterRef> rows = jdbcTemplate.query(
                "SELECT id, aspect, ux_secondary_label FROM issue_clusters WHERE id = ? AND product_id = ?",
                (rs, rowNum) -> new IssueClusterRef(
                        rs.getLong("id"),
                        rs.getString("aspect"),
                        rs.getString("ux_secondary_label")
                ),
    ...
    private Optional<Instant> resolveDefaultLaunchedAt(long productId, String uxSecondaryLabel, String aspect) {
        if ((uxSecondaryLabel == null || uxSecondaryLabel.isBlank()) && (aspect == null || aspect.isBlank())) {
            return Optional.empty();
        }
        ...
        if (uxSecondaryLabel != null && !uxSecondaryLabel.isBlank()) {
            filterSql = "AND ra.ux_secondary_label = ?";
            args.add(uxSecondaryLabel);
        } else {
            filterSql = "AND ra.aspect = ?";
            args.add(aspect);
        }
    ...
    private record IssueClusterRef(long clusterId, String aspect, String uxSecondaryLabel) {
    }
}
