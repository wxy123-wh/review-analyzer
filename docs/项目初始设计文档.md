...
本项目面向“蓝牙耳机评论改进决策”场景，目标是形成一条基于真实评论的可闭环处理链路：

1. 前端提供统一看板入口；
2. 后端负责真实评论导入、同步任务、问题洞察、趋势、词云、动作与验证；
3. NLP 服务作为独立分析能力预留；
...
- 后端：`backend/src/main/java/review/backend/BackendApplication.java`
...
- **后端 API**：负责统一业务入口，接口层位于 `backend/src/main/java/review/backend/api/`。
- **业务服务层**：负责同步、真实评论导入、评论读取、问题计算等核心逻辑，位于 `backend/src/main/java/review/backend/application/`，关键文件包括 `SyncJobService.java`、`ReviewImportService.java`、`ReviewAggregationService.java`、`InsightQueryService.java`。
- **数据访问层**：负责数据库读写、物化结果和 schema 初始化，位于 `backend/src/main/java/review/backend/data/`。
...
本设计文档选择“**问题优先级评分算法**”作为复杂功能进行算法设计，因为它是当前仓库里最具业务判断性的核心逻辑，位于 `backend/src/main/java/review/backend/application/InsightQueryService.java` 的 `listIssues()` 与 `buildIssueFactors()` 中。
...
- `productCode`
- 该商品下的评论列表（来自 `ReviewAggregationService.loadReviews(productCode)`）
...
1. **先统一评论，再做问题计算。** 分析任务先从 `ReviewAggregationService.loadReviews()` 读取真实评论，NLP 或本地回退再生成带有 `aspect` 和 `sentiment` 的物化结果。
