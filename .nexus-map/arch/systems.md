...
- key files:
  ...
  - `frontend/src/components/ProductSetupPanel.vue`: 终端采集/CSV 转换后的 raw JSONL 候选选择、路径输入兜底、商品名称输入/回填、raw JSONL 清洗、已有 taxonomy 下拉选择、只读 taxonomy 预览、商品 taxonomy 绑定、数据库导入、异步 LLM 分析启动、后端台账刷新、真实批次进度和最近 10 条评论展示；分析轮询期间如果发现物化评论数增长，会立即通知外层刷新问题、卖点、趋势和词云；当前使用页内四步导航减轻单页压力。
