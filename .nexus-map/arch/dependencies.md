...
- UX labels are not decided by the frontend or crawler. The standalone `taxonomy` module edits and saves reusable UX label systems; the data intake page only selects an existing taxonomy from the product-category dropdown, previews it readonly, and binds it to the product. Analysis uses that bound taxonomy as the allowed label set. `clean-jsonl` 的 handoff 明确要求先完成 taxonomy binding，再进入导入和分析阶段。
