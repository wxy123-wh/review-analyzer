...
- UX labels are not decided by the frontend or crawler. The frontend selects an existing taxonomy, edits its UX labels in the sidebar when needed, and binds the selected taxonomy to the product; analysis uses that taxonomy as the allowed label set. `clean-jsonl` 的 handoff 明确要求先完成 taxonomy binding，再进入导入和分析阶段。
